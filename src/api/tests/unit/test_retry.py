...
def test_retry_exponential_backoff():
    ...
    # Lenient check to account for timing variance in tests
    if len(call_times) >= 3:
...
def test_retry_respects_max_delay():
    ...
    assert result == "success"
    assert elapsed < 0.2  # Should not exceed max_delay * 2
...
def test_retry_predicate_can_stop_retries():
    """Test that retry predicate can suppress retries for matched exceptions."""
    call_count = 0
    ...
    def func():
        nonlocal call_count
        call_count += 1
        raise CustomRetryableError("do not retry")
